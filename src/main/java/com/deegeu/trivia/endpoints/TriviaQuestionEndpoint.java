...
final public class TriviaQuestionEndpoint {
    ...
    public Response getQuestion(@Context UriInfo uri, @PathParam("id") String idString) {
        ...
        if (idString.trim().equalsIgnoreCase("random")) {
           TriviaQuestion question = dataAcces.getRandomQuestion();
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response getQuestions(@Context UriInfo url, @QueryParam("offset") @DefaultValue("0") long offset) {
        ...
        long nextLink = Link.fromUri(uri.getBaseUri() + "questions?offset={offset}")
